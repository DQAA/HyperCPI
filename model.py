...
class Decoder(nn.Module):
    ...
    def forward(self, trg, src, trg_mask=None,src_mask=None):
        ...
        for layer in self.layers:
            # src = layer(src, trg, src_mask.permute(0, 1, 3, 2), trg_mask.permute(0, 1, 3, 2))
...
class Predictor(nn.Module):
    def __init__(self, encoder, decoder, device, atom_dim=34):
        ...
        self.hgnn1 = HAN(in_channels=atom_dim, out_channels=256)
        self.hgnn2 = HAN(in_channels=256, out_channels=256)
    ...
    def test(self, dataset):
        ...
        with torch.no_grad():
            for i, data in enumerate(tqdm(dataset)):
                ...
                labels.append(label)
                if i % self.batch == 0 or i == N:
                    data = pack(compounds, adjacencies, proteins, labels, self.model.device)
