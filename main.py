...
def get_dataset(dataset):
    ...
    if dataset == "celegans" or dataset == "human":
        ...
        compounds_train, adjacencies_train, proteins_train, interactions_train = load_dataset(dataset)

        """Create a dataset and split it into train/dev/test."""
        dataset = list(zip(compounds_train, adjacencies_train, proteins_train, interactions_train))
        dataset = shuffle_dataset(dataset, 1234)
        dataset_train, dataset_dev = split_dataset(dataset, 0.8)
        dataset_dev, dataset_test = split_dataset(dataset_dev, 0.5)

    elif dataset == "BindingDB":
        """Load preprocessed data."""
        compounds_train, adjacencies_train, proteins_train, interactions_train = load_dataset('BindingDB/train')
    ...
    elif dataset == "GPCR" or dataset == "kinase":
        """Load preprocessed data."""
        compounds_train, adjacencies_train, proteins_train, interactions_train = load_dataset(dataset + '/train')
        compounds_test, adjacencies_test, proteins_test, interactions_test = load_dataset(dataset + '/test')
...
if __name__ == "__main__":
    ...
    dataset_train, dataset_dev, dataset_test = get_dataset(DATASET)
